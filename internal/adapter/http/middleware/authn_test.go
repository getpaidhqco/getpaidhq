...
func TestAuthn_PublicPathsBypass(t *testing.T) {
	cases := []struct {
		...
		{"openapi spec is public", http.MethodGet, "/openapi.json", http.StatusOK, true},
		{"health only via GET", http.MethodPost, "/api/health", http.StatusUnauthorized, false},
		{"openapi spec only via GET", http.MethodPost, "/openapi.json", http.StatusUnauthorized, false},
		{"old swagger spec path is gated", http.MethodGet, "/swagger/openapi.json", http.StatusUnauthorized, false},
