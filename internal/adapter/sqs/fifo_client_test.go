...
func TestSendMessageToSQS(t *testing.T) {
	...
	if env.Get("SQS_QUEUE_URL") == "" {
		t.Skip("SQS_QUEUE_URL not set; skipping SQS integration test")
	}

	client, err := NewSQSFifoClient(logger, env)
	if err != nil {
		t.Fatalf("failed to construct SQS client: %v", err)
	}
