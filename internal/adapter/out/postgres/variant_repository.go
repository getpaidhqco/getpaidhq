...
func NewVariantRepository(primaryDb lib.Database, logger port.Logger) port.VariantRepository {
...
func (r VariantRepository) Create(ctx context.Context, variant domain.Variant) (domain.Variant, error) {
...
func (r VariantRepository) FindById(ctx context.Context, orgId string, id string) (domain.Variant, error) {
	tx := r.getTransactionFromContext(ctx)
	...
	if rows.Err() != nil {
		...
		return domain.Variant{}, rows.Err()
	}

	return variant.ToEntity(), nil
}

func (r VariantRepository) FindByProductId(ctx context.Context, orgId string, productId string, p domain.Pagination) ([]domain.Variant, int, error) {
	tx := r.getTransactionFromContext(ctx)

	var variants = make([]domain.Variant, 0)
	var count int
...
func (r VariantRepository) Update(ctx context.Context, entity domain.Variant) (domain.Variant, error) {
	tx := r.getTransactionFromContext(ctx)
