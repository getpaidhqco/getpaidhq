...
func NewProductRepository(primaryDb lib.Database, logger port.Logger) port.ProductRepository {
...
func (r ProductRepository) FindById(ctx context.Context, orgId string, id string) (domain.Product, error) {
	tx := r.getTransactionFromContext(ctx)
...
func (r ProductRepository) Find(ctx context.Context, orgId string, p domain.Pagination) ([]domain.Product, int, error) {
	tx := r.getTransactionFromContext(ctx)
