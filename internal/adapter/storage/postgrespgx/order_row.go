...
const orderColumns = `org_id, id, customer_id, reference, status, session_id, cart_id, currency, total, metadata, created_at, updated_at`

func (r *orderRow) scanInto(s scanner) error {
	return s.Scan(&r.OrgId, &r.Id, &r.CustomerId, &r.Reference, &r.Status, &r.SessionId,
		&r.CartId, &r.Currency, &r.Total, &r.Metadata, &r.CreatedAt, &r.UpdatedAt)
}
