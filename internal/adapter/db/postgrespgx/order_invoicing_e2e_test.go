...
// Postgres (testcontainer via poolForTest(t)), driving a REAL wired InvoiceService
// (with a real InvoiceSettingsService resolver and the real pgx repos) through
// OrderService.CreateOrder/CompleteOrder.
//
// This closes the gap left by coupon_billing_e2e_test.go, which passes noop for
...
// BuildForOrder on a real pgx DB) is exercised against Postgres. We prove the
...
func TestOrderInvoicing_MixedCart_DirectPayment_E2E(t *testing.T) {
	...
	subProductId, subPriceId := seedSubscriptionPrice(t, pool, orgId, 0)
	// Overwrite the seeded $100/minute price's cadence to a clean monthly $100.
	_, err := pool.Exec(ctx,
		"UPDATE prices SET unit_price=$1, billing_interval=$2, cycles=$3 WHERE org_id=$4 AND id=$5",
		10000, string(domain.BillingIntervalMonth), 0, orgId, subPriceId)
	require.NoError(t, err)
	oneTimeProductId, oneTimePriceId := seedOneTimePrice(t, pool, orgId, 5000)
	...
	invoices := buildWiredInvoiceService(t, pool)
	orders := buildWiredOrderService(t, pool, coupons, invoices)
	...
	inv, err := NewInvoiceRepo(pool).FindOrderInvoice(ctx, orgId, orderId)
	require.NoError(t, err)
	...
	all, total, err := NewInvoiceRepo(pool).List(ctx, orgId, domain.Pagination{Page: 1, Limit: 50})
	require.NoError(t, err)
	require.Equal(t, 1, total, "exactly one invoice for the order")
	require.Len(t, all, 1)

	// The persisted Payment links to that invoice.
	payments, ptotal, err := NewPaymentRepo(pool).FindBySubscriptionId(ctx, orgId, sub.Id, domain.Pagination{Page: 1, Limit: 10})
	require.NoError(t, err)
...
func TestOrderInvoicing_OneTimeCoupon_DirectPayment_E2E(t *testing.T) {
	...
	productId, priceId := seedOneTimePrice(t, pool, orgId, 20000)
	customer := seedCustomer(t, pool, orgId)
	pspConfigId := seedMemoryPsp(t, pool, orgId)
	pm := seedPaymentMethod(t, pool, orgId, customer.Id)
	...
	ds, err := NewDiscountRepo(pool).ActiveForOrder(ctx, orgId, orderId)
	require.NoError(t, err)
...
func TestOrderInvoicing_UpfrontInvoice_E2E(t *testing.T) {
	...
	productId, priceId := seedOneTimePrice(t, pool, orgId, 7500)
	customer := seedCustomer(t, pool, orgId)
	pspConfigId := seedMemoryPsp(t, pool, orgId)
	pm := seedPaymentMethod(t, pool, orgId, customer.Id)
	...
	persistedUpfront, err := NewInvoiceRepo(pool).FindOrderInvoice(ctx, orgId, orderId)
	require.NoError(t, err)
	...
	settled, err := NewInvoiceRepo(pool).FindOrderInvoice(ctx, orgId, orderId)
	require.NoError(t, err)
	...
	_, total, err := NewInvoiceRepo(pool).List(ctx, orgId, domain.Pagination{Page: 1, Limit: 50})
	require.NoError(t, err)
...
func TestOrderInvoicing_UpfrontInvoiceWithCoupon_E2E(t *testing.T) {
	...
	res, err := NewCouponReservationRepo(pool).FindByOrder(ctx, orgId, orderId)
	require.NoError(t, err)
	require.Len(t, res, 1, "CreateOrder holds one reservation")
	ds0, err := NewDiscountRepo(pool).ActiveForOrder(ctx, orgId, orderId)
	require.NoError(t, err)
...
func TestOrderInvoicing_CustomInvoiceSettings_E2E(t *testing.T) {
	...
	productId, priceId := seedOneTimePrice(t, pool, orgId, 4200)
	customer := seedCustomer(t, pool, orgId)
	pspConfigId := seedMemoryPsp(t, pool, orgId)
	pm := seedPaymentMethod(t, pool, orgId, customer.Id)

	settings := service.NewInvoiceSettingsService(NewSettingRepo(pool), noopLogger{})
	...
	invoices := service.NewInvoiceService(
		NewInvoiceRepo(pool), NewOrderRepo(pool), NewPriceRepo(pool), NewSubscriptionRepo(pool),
		buildUsageService(t, pool), NewTxManager(pool), noopLogger{},
		NewDiscountRepo(pool), NewCouponRepo(pool), NewCouponReservationRepo(pool), settings,
	)
	orders := buildWiredOrderService(t, pool, coupons, invoices)
	...
	inv, err := NewInvoiceRepo(pool).FindOrderInvoice(ctx, orgId, created.Order.Id)
	require.NoError(t, err)
...
func TestOrderInvoicing_Webhook_MixedCart_E2E(t *testing.T) {
	...
	invoices := buildWiredInvoiceService(t, pool)
	orders := buildWiredOrderService(t, pool, coupons, invoices)
	workflow := buildWiredOrderWorkflowService(t, pool, coupons, invoices)
...
func TestOrderInvoicing_Webhook_OneTimeCoupon_E2E(t *testing.T) {
	...
	payments, ptotal, err := NewPaymentRepo(pool).List(ctx, orgId, domain.Pagination{Page: 1, Limit: 10})
	require.NoError(t, err)
	require.Equal(t, 1, ptotal)
