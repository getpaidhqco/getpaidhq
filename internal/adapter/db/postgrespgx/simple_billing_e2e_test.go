...
func TestSimpleSubscriptionBilling_E2E(t *testing.T) {
	...
	items, err := NewOrderRepo(pool).FindOrderItemsByOrderId(ctx, orgId, sub.OrderId)
	require.NoError(t, err)
	require.Len(t, items, 1)
	price, err := NewPriceRepo(pool).FindById(ctx, orgId, items[0].PriceId)
	require.NoError(t, err)
	...
	inv, err := NewInvoiceRepo(pool).FindBySubscriptionCycle(ctx, orgId, sub.Id, 0)
	...
	payments, total, err := NewPaymentRepo(pool).FindBySubscriptionId(ctx, orgId, sub.Id, domain.Pagination{Page: 1, Limit: 10})
	require.NoError(t, err)
	...
	persisted, err := NewSubscriptionRepo(pool).FindById(ctx, orgId, sub.Id)
	require.NoError(t, err)
