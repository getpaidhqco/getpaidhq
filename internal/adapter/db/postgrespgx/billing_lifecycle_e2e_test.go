...
func TestMultiCycleBilling_CarryOver_E2E(t *testing.T) {
	...
	advanced, err := NewSubscriptionRepo(pool).FindById(ctx, orgId, fx.sub.Id)
	require.NoError(t, err)
	...
	invJun, err := NewInvoiceRepo(pool).FindBySubscriptionCycle(ctx, orgId, fx.sub.Id, 0)
	require.NoError(t, err)
	...
	invJul, err := NewInvoiceRepo(pool).FindBySubscriptionCycle(ctx, orgId, fx.sub.Id, 1)
	require.NoError(t, err)
...
func TestTrialSubscription_FlatFeeWaived_E2E(t *testing.T) {
	...
	flat := domain.Price{
		OrgId: orgId, Id: ids.Generate("price"), VariantId: seedVariantChain(t, pool, orgId),
		Label:    "Platform fee",
	...
	_, err := NewPriceRepo(pool).Create(ctx, flat)
	require.NoError(t, err)
	flatItem := seedOrderItem(t, pool, orgId, fx.order.Id, flat.Id)
	_, err = pool.Exec(ctx, "UPDATE order_items SET subscription_id=$1 WHERE org_id=$2 AND id=$3", fx.sub.Id, orgId, flatItem.Id)
	require.NoError(t, err)

	fx.sub.Status = domain.SubscriptionStatusTrial
	_, err = pool.Exec(ctx, "UPDATE subscriptions SET status=$1 WHERE org_id=$2 AND id=$3", string(domain.SubscriptionStatusTrial), orgId, fx.sub.Id)
	require.NoError(t, err)
	...
	inv, err := buildInvoiceService(t, pool).BuildForBillingPeriod(ctx, fx.sub)
	require.NoError(t, err)
...
func TestCreateSubscriptionsForOrder_MeteredCadenceClamp_E2E(t *testing.T) {
	...
	_, err := NewCustomerRepo(pool).Create(ctx, cust)
	require.NoError(t, err)
	...
	_, err = NewMeterRepo(pool).Create(ctx, meter)
	require.NoError(t, err)
	...
	_, err = NewPriceRepo(pool).Create(ctx, price)
	require.NoError(t, err)

	order := seedOrder(t, pool, orgId, cust.Id)
	item := seedOrderItem(t, pool, orgId, order.Id, price.Id)

	subs, err := buildSubscriptionService(t, pool).CreateSubscriptionsForOrder(ctx, orgId, order.Id)
	require.NoError(t, err)
	...
	linked, err := NewOrderRepo(pool).FindOrderItemsBySubscriptionId(ctx, orgId, subs[0].Id)
	require.NoError(t, err)
...
func TestZeroUsage_ZeroAmountInvoice_E2E(t *testing.T) {
	...
	result, err := buildSubscriptionService(t, pool).ChargeForBillingPeriod(ctx, fx.sub)
	require.NoError(t, err)
