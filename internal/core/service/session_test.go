...
func TestSessionService_CreateSession(t *testing.T) {
	t.Run("creates a cart then a session bound to it, and publishes", func(t *testing.T) {
		...
		got, err := svc.CreateSession(context.Background(), domain.CreateSessionInput{OrgId: "org_1", Currency: "USD"})
	...
	t.Run("cart create failure aborts before session and event", func(t *testing.T) {
		...
		_, err := svc.CreateSession(context.Background(), domain.CreateSessionInput{OrgId: "org_1", Currency: "USD"})
