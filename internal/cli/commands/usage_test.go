...
func TestMetersCmd(t *testing.T) {
	...
	runCases(t, []cmdCase{
		// ----------------------------------------------------------------
		// meters create — full body via --data (passthrough: only supplied fields)
		// ----------------------------------------------------------------
		{
		...
		// create via flags — group_by omitted yields [] (cobra StringArray default)
		{
			name:       "create with flags",
			args:       []string{"meters", "create", "--code", "bytes_used", "--name", "Bytes Used", "--aggregation", "sum", "--field", "bytes", "--carry-over"},
			wantMethod: "POST",
			wantPath:   "/api/meters",
			wantBody:   `{"code":"bytes_used","name":"Bytes Used","aggregation":"sum","field_name":"bytes","carry_over":true,"rounding_mode":"","rounding_scale":0,"filters":null,"group_by":[],"metadata":null}`,
			respBody:   `{"id":"met_2","code":"bytes_used","name":"Bytes Used","aggregation":"sum","field_name":"bytes","carry_over":true,"rounding_mode":"","rounding_scale":0,"filters":[],"group_by":null,"metadata":null,"created_at":"2026-06-12T10:00:00Z","updated_at":"2026-06-12T10:00:00Z"}`,
...
func TestUsageCmd(t *testing.T) {
	...
	runCases(t, []cmdCase{
		// ----------------------------------------------------------------
		// usage ingest — happy path: body assert (events array shape)
		// ----------------------------------------------------------------
		{
			...
			wantBody:   `{"events":[{"customer_id":"cus_1","external_customer_id":"","metric_code":"api_calls","subscription_id":"","external_id":"","timestamp":"2026-06-12T10:00:00Z","metadata":{"region":"eu"}}]}`,
			respBody:   ingestResp,
			wantOut:    []string{`"status": "recorded"`},
			wantCode:   0,
		},
		// ingest with external_customer_id
		{
			...
			wantMethod: "POST",
			wantPath:   "/api/usage/ingest",
			wantBody:   `{"events":[{"customer_id":"","external_customer_id":"ext_cus_42","metric_code":"bytes_used","subscription_id":"","external_id":"dedup_key_1","timestamp":"0001-01-01T00:00:00Z","metadata":null}]}`,
			respBody:   ingestResp,
		...
		// ingest via --data (batch passthrough)
		{
