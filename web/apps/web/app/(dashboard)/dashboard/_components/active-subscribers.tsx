...
import {fetchActiveSubscribers} from "@/app/(dashboard)/dashboard/data";
...
export function ActiveSubscribers({startDate, endDate}: Props) {
  const id = useId();
  ...
  const query = useQuery<RecurringRevenue[] | undefined>({
    queryKey: ['active_subscribers', startDate, endDate],
    queryFn: async () => fetchActiveSubscribers({
  ...
  useEffect(() => {
    if (query.data) {
      ...
      setTotal(parseFloat((query.data[query.data.length - 1]?.total  || 0).toFixed(0)))
  ...
  const firstMonth = chartData[0]?.period as string;
  const lastMonth = chartData[chartData.length - 1]?.period as string;
