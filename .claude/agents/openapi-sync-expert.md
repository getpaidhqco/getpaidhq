---
name: openapi-sync-expert
description: Use this agent when you need to update, synchronize, or validate the OpenAPI specification (openapi.yml) against the actual REST API implementation in internal/api. This includes adding new endpoints, updating request/response schemas, fixing discrepancies between the spec and code, or ensuring complete API documentation coverage. Examples: <example>Context: The user has just added a new endpoint to the API and needs to update the OpenAPI spec accordingly.\nuser: "I've added a new POST /api/v1/customers/{id}/payment-methods endpoint"\nassistant: "I'll use the openapi-sync-expert agent to update the OpenAPI specification with this new endpoint"\n<commentary>Since a new API endpoint was added, use the openapi-sync-expert agent to ensure the OpenAPI spec reflects this change.</commentary></example> <example>Context: The user wants to ensure the OpenAPI spec is complete and accurate.\nuser: "Can you check if our OpenAPI spec is up to date with all the endpoints?"\nassistant: "I'll use the openapi-sync-expert agent to audit and synchronize the OpenAPI specification with the current API implementation"\n<commentary>The user is asking for OpenAPI validation, so use the openapi-sync-expert agent to check and update the spec.</commentary></example> <example>Context: The user has modified response DTOs and needs the spec updated.\nuser: "I've updated the CustomerResponse DTO to include a new 'status' field"\nassistant: "I'll use the openapi-sync-expert agent to update the OpenAPI specification to reflect the new response schema"\n<commentary>Since API response structures have changed, use the openapi-sync-expert agent to update the corresponding schemas in the OpenAPI spec.</commentary></example>
color: yellow
---
