...
func TestAdminCommands(t *testing.T) {
	runCases(t, []cmdCase{
		...
		// api-keys list — ListResponse {data, meta}
		{
			name:       "api-keys list",
		...
		// api-keys revoke — DELETE returns 200 + EmptyResponse body per spec;
		// renderDeleted prefixes "api-key " and suffixes " deleted".
		{
			...
			respBody:   `{}`,
			wantOut:    []string{"api-key key_1 deleted"},
		},
		{
		...
		// gateways create — CRITICAL: credentials pass through as literal strings
		// (CreateGatewayRequestCredentials is a plain map[string]string, not
		// domain.Secret), so they are NOT redacted. config omitted when unset.
		{
			...
			wantBody:   `{"name":"prod-paystack","psp":"paystack","credentials":{"secret_key":"sk_live_x"}}`,
		...
		// The spec's list endpoint has no parent_id query param, so --parent is a
		// no-op on the wire; assert the request still hits GET /api/settings.
		{
		...
		// settings update — type optional, omitted when unset
		{
		...
		// settings delete — DELETE returns 200 + EmptyResponse body per spec;
		// renderDeleted prefixes "setting " and joins parent/id with "/".
		{
			...
			wantPath:   "/api/settings/ui/color",
			respBody:   `{}`,
			wantOut:    []string{"setting ui/color deleted"},
		},
		{
		...
		// webhooks create — secret optional, omitted when unset
		{
			...
			wantBody:   `{"url":"https://example.com/hook","events":["subscription.created","payment.succeeded"]}`,
