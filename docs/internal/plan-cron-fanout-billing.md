...
-1. **Stale `spawn-subscription-runner` step in `payment_success.go`** — the Hatchet payment-success DAG still has a `spawn-subscription-runner` task that calls the now-no-op `StartSubscriptionWorkflow`. Benign (the initial payment already charged; the cron handles renewal; no functional gap), but the step name/intent is misleading. Follow-up: remove that step from the payment-success DAG (and confirm the Temporal `PaymentSuccessWorkflow` equivalent — engine parity).
