...
CREATE TABLE coupon_reservations (
  ...
  coupon_code_id TEXT,                     -- nullable: NULL = programmatic / code-less hold
  customer_id    TEXT NOT NULL,
  order_id       TEXT NOT NULL,            -- the holder; the subscription is known at convert time
  expires_at     TIMESTAMP(3) NOT NULL,    -- checkout-hold TTL
  created_at     TIMESTAMP(3) NOT NULL DEFAULT CURRENT_TIMESTAMP,

  CONSTRAINT coupon_reservations_pkey PRIMARY KEY (org_id, id),
  CONSTRAINT coupon_reservations_org_coupon_order_key UNIQUE (org_id, coupon_id, order_id),
...
| Once-per-customer | `coupon.once_per_customer` | block if a `Discount` **or** live reservation exists for `(coupon_id, customer_id)` |
...
```go
// Reserve runs the atomic, reservation-aware gate (§4.1) and inserts a hold.
// Called inside the CreateOrder tx; a refusal aborts the whole order.
...
// deletes the reservation, creates the Discount (StartCycle from input), increments
// code.times_redeemed. Called on payment success. Joins the caller's tx via ctx.
...
func (s *CouponService) Release(ctx, orgId, orderId string) error

// PreviewForOrder computes the discount the reserved coupon would apply to the given lines at a
// cycle — used to build the hosted cycle-0 invoice from the reservation, pre-payment. Pure read.
func (s *CouponService) PreviewForOrder(ctx, orgId, orderId string, lines []domain.DiscountableLine, cycle int, currency string) (DiscountPreview, error)
```

```go
type ReserveInput struct { OrgId, Code, CouponId, CustomerId, OrderId, Currency string; Amount int64 }
type ConsumeInput struct { OrgId, OrderId, SubscriptionId string; StartCycle int }
```
...
type CouponReservationRepository interface {
    ...
    FindByOrder(ctx, orgId, orderId string) ([]domain.CouponReservation, error)
    DeleteByOrder(ctx, orgId, orderId string) error
...
- Inside `OrderService.CreateOrder`'s existing tx, after the order row exists, call
  `CouponService.Reserve(...)` with `OrderId`, `CustomerId`, the cart currency, and the cart
  subtotal (for the `MinimumAmount` restriction).
- **A reservation refusal fails the entire order** — the tx rolls back, no order/subscription is
  created, and the handler returns the refusal reason as an `ApiError`. (Per decision: a coupon
  that can't be reserved aborts the order.)
...
- **Synchronous** `OrderService.CompleteOrder`: inside its existing tx, after activating the
  subscription, call `CouponService.Consume(orderId, subscriptionId, StartCycle: subscription's
  current cycle)`. The `Discount` commits in the same tx, *before* the post-commit
...
On a failed/cancelled order completion, call `CouponService.Release(orgId, orderId)`. Abandoned
orders that never resolve are handled by lazy expiry (no explicit release needed).
...
| `core/service` | `CouponService.Reserve/Consume/Release/PreviewForOrder`; gate made reservation-aware; `InvoiceService` discount application; `OrderService` reserve/convert/release hooks. |
