...
**Engine-parity rule (load-bearing):** every change to workflow / billing / dunning / reminder behavior MUST be implemented and verified on **both** adapters (`internal/adapter/hatchet/` and `internal/adapter/temporal/`) — a change that lands on one engine silently breaks the other when `WORKFLOW_ENGINE` is switched. Keep engine-agnostic logic in `core/` so both share it; Temporal reaches shared services through **activities** (workflow code must stay deterministic), not direct repo calls. The one documented exception is the billing-trigger model itself: Hatchet uses a cron + per-org fan-out into bounded per-renewal tasks (forced by Hatchet's creation-date-partitioned durable-log retention, which kills immortal tasks), while Temporal keeps the durable per-subscription `SubscriptionWorkflow` with `ContinueAsNew`. See `docs/internal/plan-cron-fanout-billing.md` and `docs/internal/durable-runner-timeouts.md`.
